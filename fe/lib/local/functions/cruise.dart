...
void filterCruises(ctx) {
  ...
  if (cruiseItinerary.value.isNotEmpty)
    itinerary = itinerary.where((element) {
      var rule = true;
      for (var item in cruiseItinerary.value) {
        rule = rule ||
  ...
  try {
    var processedData = buildCruiseDataTable(ctx);
...
///
List buildCruiseDataTable(context) {
  try {
    var columns = ["description"];
    var header = getCruiseHeader(context, cruiseResults.value, columns);
    var detail = getCruiseDetail(context, cruiseResults.value, columns);
          ...
          children: [
            ...
            CustomFormCalendarFieldWidget(
                ...
                initialEndDate: getNextCruiseDate()
                    .add(Duration(days: parseInt(cruiseDay.value) - 1)),
                minimumDate: arrivalDate.value,
                maximumDate: getNextCruiseDate()
                    .add(Duration(days: parseInt(cruiseDay.value))),
...
void showCruiseDetailDialog(context, row) {
  showCustomDialog(context, CruiseDetailWidget(row: row), "Close",
      backgroundColor: Color.fromARGB(100, 0, 0, 0), buttonColor: Colors.white);
}

promteCruise(formKey) {
  try {
