...
void autoFillDestination(destination, index, type, days) {
  ...
  setFormValue(globalctx.memory["destinations"], index, "travel_rhythm",
      destination.toString().contains("cruise") ? "3" : "1");
  setFormValue(globalctx.memory["destinations"], index, "explorationMode",
      destination.toString().contains("cruise") ? "1" : "0");
  addDestination(destination);
...
void processDestinationPromotion(destination, index, type) {
  ...
  if (destination == arrival["description"] &&
      index == 0 &&
      type == "arrival") {
    copyArrivalHotels(departure["description"],
        globalctx.destinations.length - 1, "departure");
  }
...
/// @return void
///
void promoteDestination(_formKey, destination, index, type) {
...
Future processDestinations(context) async {
  ...
  if (globalctx.promotedDestinations.isNotEmpty & (dayleft.value == 0)) {
    ...
    for (var destination in destinations.entries.toList()) {
      ...
      var days = parseInt(explorationDay);
...
RxBool validateDragDestinationOptions(destination, index, type) {
  ...
  bool isGalapagosCruise = destination.toString().contains("cruise");
  bool isDeparture =
...
void deleteGraphDragDestinationOption(String destination) {
  if (arrivalPort.value != getDestinationIdByName(destination) &&
    ...
    var destDay = parseInt(getFormValue(globalctx.memory["destinations"],
    ...
    if (destination.toString().contains("cruise")) {
...
void updateTotalLeftAccumulated() {
  try {
    accumulated.value = 0;
    globalctx.memory["destinations"].forEach((key, value) {
      accumulated.value +=
          parseInt(globalctx.memory["destinations"][key]["explorationDay"]);
    });
    updateTotalDays();
  } catch (e) {
    log(e);
...
getDestinationTravelRhythm(destination, type) {
  ...
  var trRange = trData
      .firstWhere((e) => e["code"] == parseInt(destData["travel_rhythm"]));
  return trRange;
}
...
int getDestinationExplorationDay(index) {
  ...
  if (destinations[index] != null) {
    if (destinations[index]["explorationDay"] != null) {
      result = parseInt(destinations[index]["explorationDay"]);
    }
  }
  return result;
}

/// ## updateCurrentDestinationType
...
/// @return void
///
void updateCurrentDestinationType() {
  var type = "tour";
...
///### Uses:
/// ```dart
///     (!destination.toString().contains("cruise") &&
...
void updateDestinationDays() {
  ...
  for (var dest in globalctx.memory["destinations"].entries) {
    ...
    destDays[parseInt(destDay["index"])] =
        (parseInt(destDay["explorationDay"]));
  }
}
...
int getCurrentDestinationDayId(int destId, int destDay) {
  int dayId = 0;
  var realAccumulated = 0;
  try {
    var destinations = globalctx.memory["destinations"];
    var destIndex = parseInt(destinations[destId.toString()]["index"]);
    for (var i = 0; i <= destIndex; i++) {
      var destination = destinations[i.toString()];
      var explorationDay = parseInt(destination.value["explorationDay"]);
      realAccumulated += explorationDay;
    }
    for (var i = 0; i < destinations.length; i++) {
      var destination = destinations[i.toString()];
      var explorationDay = parseInt(destination.value["explorationDay"]);
      dayId += parseInt(explorationDay - 1 + realAccumulated);

      if (explorationDay <= realAccumulated) {
        return dayId - 1 + destDay;
      }
    }
