...
Future paginateDay(context) async {
  if (globalctx.memory["promoted"] != null &&
    ...
    showCustomDialog(context,
        DestinationDetailLeftWidget(destination: "quito", index: 0), "Close");
  }
}
...
decideBypass(direction) {
  ...
  int explorationMode = parseInt(getFormValue(
      globalctx.memory["destinations"], index, "explorationMode", "0"));
  var explorationDay = parseInt(getFormValue(
...
getDestinationIndexByDay() {
  ...
  for (var _dest in _destinations.entries) {
    var _destData = destinations[_dest.key];
    var explorationDay = parseInt(_destData["explorationDay"]);
    _accumulated += explorationDay;
    if (explorationDay <= _accumulated) {
      destinationIndex = parseInt(_dest.key);
    }
    index++;
  }
...
/// *__Method to get the current explorationDay gived a destination index from memory__*
///
///### Uses:
/// ```dart
...
///```
getCurrentExplorationDay() {
  int index = getDestinationKeyByDay();
  int explorationDay = parseInt(getFormValue(
      globalctx.memory["destinations"], index, "explorationDay", "0"));
...
bypassCruise(String direction, index) {
  try {
    int explorationDay = parseInt(getFormValue(
        globalctx.memory["destinations"], index, "explorationDay", "0"));
    int arrivalExplorationDay = parseInt(getFormValue(
...
processDays() {
  ...
  for (String index in destinations.keys) {
    ...
    var explorationDay = parseInt(destData["explorationDay"]);
    var destination =
        globalctx.states["destinations"][parseInt(index)]["destination"];
  ...
  if (result.isNotEmpty) {
    ...
    if (!clearedHours[currentDay.value]) {
      updateCurrentDestinationTypeTravelRhythm();
    }
...
void saveExplorationDay(int index, int val0, int val1, {String? key}) {
  try {
    ...
    int total0 = parseInt(getFormValue(
        globalctx.memory["destinations"], index, "explorationDay", "0"));
