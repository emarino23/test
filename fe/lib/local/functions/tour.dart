...
Future processTour() async {
  try {
    ...
    for (String dest in destinations.keys.toList()) {
      ...
      for (var i = 0; i < parseInt(explorationDay); i++) {
        Map myDayDto = dayDto;
...
preparePayloadToSend() {
  ...
  globalctx.memory["logistic"]["arrival_date"] = arrivalDate.value;
  ...
  if (translatingService.value.isNotEmpty) {
    var passengers = globalctx.memory["tour"]["passengers"] ?? "1";
    globalctx.payload["tour"]["passengers"] =
        (parseInt(passengers) + getGuides()).toString();
  }
}
